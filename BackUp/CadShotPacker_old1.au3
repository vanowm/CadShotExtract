...
;~ _FileSysMonDirEventHandler()copy
	...
	If $modified > 0 And TimerDiff($modified) > 1000 And _FileTestOpen($cadShotImage) Then
		$modified = 0
		checkFile()
	EndIf
...
func checkFile()
	...
	If Not FileExists($cadShotPpi) Then FileWrite($cadShotPpi, "155")
...
Func _FileTestOpen($FileIn)
	...
	EndIf
EndIf
debug("FileTestOpen", $return, _WinAPI_CreateFileEx("\\?\" & $FileIn, 3, 2, 7, 0x02000000))
return $return
